...
const rolesList = [
  { id: 'super_admin', name: "Super Admin", count: 2, color: "#D81E5B", desc: "Full system access. Can modify roles and global platform settings." },
              ...
              <div className="p-6 border-b border-zinc-50 flex items-center justify-between">
                 <div className="flex items-center gap-3">
                    <div className="p-2.5 bg-zinc-950 text-white rounded-xl">
                       {activeRole === 'super_admin' ? <ShieldCheck className="w-5 h-5" /> : <Settings2 className="w-5 h-5" />}
                    </div>
                    <div>
                 ...
                 <div className="flex items-center gap-2">
                    ...
                    <Button className="bg-[#D81E5B] hover:bg-[#D81E5B]/90 text-white h-10 px-6 rounded-xl font-bold shadow-lg shadow-[#D81E5B]/20 transition-all">
                             ...
                             {actions.map(action => (
                                <td key={`${module.id}-${action.id}`} className="px-4 py-6 text-center">
                                   <div className="flex justify-center">
                                      <PermissionToggle 
                                        isActive={activeRole === 'super_admin' || (activeRole === 'admin' && action.id !== 'delete')} 
                                      />
                                   </div>
                                </td>
                             ))}
...
function PermissionToggle({ isActive }: { isActive: boolean }) {

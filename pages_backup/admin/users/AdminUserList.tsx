...
export default function AdminUserList() {
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      // 3. Handle Agents table replication if role became agent/admin/superadmin
      const rolesToReplicate = ["agent", "admin", "superadmin"];
      if (rolesToReplicate.includes(editingUser.role)) {
        ...
        if (!existingAgent) {
          ...
          const { error: agentError } = await supabase
            ...
            .insert([{
              ...
              territory: "Colombo", // Default
              performance_score: 0
            }]);
          if (agentError) throw agentError;
        }
      } else if (currentUser && rolesToReplicate.includes(currentUser.role)) {
        ...
        // await supabase.from("agents").delete().eq("user_id", editingUser.id);
      }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#1A1C29] tracking-tight">Identity Directory</h1>
                  ...
                  <tr key={user.id} className="hover:bg-zinc-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <Badge className={`rounded-lg px-2.5 py-0.5 text-[11px] font-bold shadow-none border-none capitalize ${
                         (user.role || "").includes('admin') ? 'bg-[#1A1C29] text-white' : 
                         user.role === 'salon_owner' ? 'bg-[#D81E5B]/10 text-[#D81E5B]' :
                         user.role === 'customer' ? 'bg-zinc-100 text-zinc-600' :
          ...
          <div className="p-8 space-y-6 bg-white">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger className="h-12 bg-zinc-50 border-none rounded-xl font-medium focus:ring-2 focus:ring-[#D81E5B]/20">

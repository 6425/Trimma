...
-- Expect pending 22 / listed 658 (or current live totals)
SELECT
  public.published_listing_count() AS listed_published,
  public.listing_generation_queue_payload() -> 'pendingCount' AS pending_captured,
  public.listing_generation_queue_payload() -> 'listedCount' AS listed_in_payload,
  jsonb_array_length(public.listing_generation_queue_payload() -> 'rows') AS queue_rows,
  jsonb_array_length(public.published_marketplace_listings()) AS marketplace_rows;

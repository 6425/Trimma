-- Trimma AI: Global Service Architecture Update
-- This script:
-- 1. Corrects the Categories Data (with proper SQL escaping)
-- 2. Creates the Global Services Catalog table
-- 3. Updates the existing Services table to support Global Templates

-- ==========================================
-- 1. FIX CATEGORIES (Correcting the single quote issue)
-- ==========================================
INSERT INTO categories (id, name, slug, icon)
VALUES 
  ('e573447d-35c9-40d7-8a71-661fd05ba3a1', 'Barber Salon', 'barber-salon', 'Scissors'),
  ('ea03b018-c45f-483a-9857-77058ac7c3ce', 'Beauty Parlours', 'beauty-parlours', 'Sparkles'),
  ('6bd05eaf-ee0c-4f69-91eb-b14ac453c143', 'Bridal & Beauty', 'bridal-and-beauty', 'Heart'),
...
-- ==========================================
-- 3. LINK EXISTING SERVICES TO GLOBAL CATALOG
-- ==========================================
-- Add global_service_id to the existing services table
DO $$ 
BEGIN 
...
-- Optional: Adding an index for faster filtering of global templates
CREATE INDEX IF NOT EXISTS idx_services_global_id ON services(global_service_id);

-- ==========================================
-- 4. INSERT SOME INITIAL GLOBAL SERVICES (TEMPLATES)
-- ==========================================
...
VALUES 
  ...
  ('Beard Trim', 'beard-trim', 'e573447d-35c9-40d7-8a71-661fd05ba3a1', 800.00, 15, 'User'),
  ('Bridal Makeup', 'bridal-makeup', '6bd05eaf-ee0c-4f69-91eb-b14ac453c143', 25000.00, 120, 'Heart'),
  ('Manicure', 'manicure', '7faebaac-aa28-41cc-867c-5451b39b68d6', 2000.00, 45, 'Paintbrush')

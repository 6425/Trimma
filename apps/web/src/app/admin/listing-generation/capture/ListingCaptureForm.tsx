...
export function ListingCaptureForm({ categories, servicesByCategoryId }: Props) {
  ...
  const [manualListing, setManualListing] = useState<BusinessListingFormState>(() =>
    emptyBusinessListingForm(categories[0]?.id || "")
  );
  ...
  const updateManualListing = (updates: Partial<BusinessListingFormState>) => {
  ...
  const handleManualListingSave = async () => {
    ...
    try {
      ...
      setManualListing(emptyBusinessListingForm(categories[0]?.id || ""));

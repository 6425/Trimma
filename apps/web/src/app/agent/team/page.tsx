    ...
    try {
      ...
      if (!result.success) {
        ...
        if (result.error?.includes("authenticated")) {
          router.replace("/login?redirectTo=/agent/team");
        }
        return;
      }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            View field agents on your team and set what share they receive from your agent commission on bookings and subscriptions.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 overflow-hidden">
                ...
                {subAgents.map((row) => (
                  <tr key={row.id} className="border-t border-zinc-50 hover:bg-zinc-50/60">
                    ...
                    <td className="px-4 py-3 text-right font-medium text-zinc-700">
                      {formatLKR(row.bookingGross)}
                    </td>
                    <td className="px-4 py-3 text-right font-bold text-emerald-700">
                      {formatLKR(row.bookingEarnings)}
                    </td>
                    <td className="px-4 py-3 text-right font-medium text-zinc-700">
                      {formatLKR(row.subscriptionGross)}
                    </td>
                    <td className="px-4 py-3">
          ...
          <div className="px-4 py-3 border-t border-zinc-100 bg-zinc-50 text-xs text-zinc-500 flex items-start gap-2">
            ...
            <span>
              ...
              Default split is 50% and applies to new bookings and subscriptions only.
            </span>
          </div>
        </div>
      )}

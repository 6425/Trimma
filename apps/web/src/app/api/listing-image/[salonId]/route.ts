...
async function fetchImage(value: string): Promise<Response | null> {
...
async function findCurrentGoogleBusinessImage(placeId: string): Promise<string | null> {
  ...
  return (
    candidates.find((url) => {
      ...
      return Boolean(dimensions && Number(dimensions[1]) >= 300 && Number(dimensions[2]) >= 200);
    }) || null
  );
}
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  if (savedImage) {
    const savedResponse = await fetchImage(savedImage);
  ...
  if (placeId) {
    ...
    if (currentImage) {
      const currentResponse = await fetchImage(currentImage);

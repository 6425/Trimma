...
export function SalonDashboardGuideCard() {
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl border border-[#f9e000]/25 bg-zinc-950 text-white shadow-sm">
      <BookOpen className="absolute -right-6 -top-6 w-32 h-32 text-[#f9e000]/8 pointer-events-none" />
      <div className="relative z-10 p-5 lg:p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
          <div className="flex items-start gap-4 min-w-0">
            <div className="w-12 h-12 rounded-2xl bg-[#f9e000]/15 border border-[#f9e000]/30 flex items-center justify-center shrink-0">
              <FileText className="w-6 h-6 text-[#f9e000]" />
            </div>
            <div className="min-w-0">
              <span className="inline-flex items-center gap-1.5 bg-[#f9e000]/15 text-[#f9e000] border border-[#f9e000]/25 px-2.5 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wider mb-2">
          ...
          {LANG_OPTIONS.map(({ code, native }) => {
            ...
            return (
              <a
                ...
                download
                className="group flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 hover:border-[#f9e000]/40 hover:bg-[#f9e000]/10 transition-colors"
              >
                <div className="min-w-0">
                  <div className="text-[10px] font-black uppercase tracking-wider text-[#f9e000]">{native}</div>
                  <div className="text-xs font-semibold text-white/90 truncate mt-0.5">
                    {doc?.title || `Salon Owner Handbook (${code})`}
                  </div>
                </div>
                <Download className="w-4 h-4 text-white/50 group-hover:text-[#f9e000] shrink-0" />
              </a>
            );
          })}
        </div>

...
import {
  getPortalGuideFallbacks,
  resolvePortalGuideDocuments,
  type PortalGuideDocument,
} from "@/lib/portal-guide-fallback";
...
type PortalGuideDownloadsProps = {
  documentType: "agent_guide" | "regional_head_guide";
  title: string;
  description: string;
  formatLabel?: "Word" | "PDF";
};

export function PortalGuideDownloads({
  documentType,
  title,
  description,
  formatLabel = "Word",
}: PortalGuideDownloadsProps) {
  const [docs, setDocs] = useState<PortalGuideDocument[]>(() => getPortalGuideFallbacks(documentType));
  const [copiedSlug, setCopiedSlug] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    const staticFallbacks = getPortalGuideFallbacks(documentType);
    void Promise.resolve().then(() => {
      fetch(`/api/public/help-documents?document_type=${documentType}`)
        .then(async (r) => {
          if (!r.ok) return staticFallbacks;
          const data = (await r.json()) as { documents?: PortalGuideDocument[] };
          return resolvePortalGuideDocuments(documentType, data.documents);
        })
        .then((resolved) => {
  ...
  const copyLink = async (doc: PortalGuideDocument) => {
    ...
    <section id="guides" className="scroll-mt-24 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="p-6 sm:p-8 border-b border-slate-100">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-zinc-900 tracking-tight mb-1">{title}</h2>
            <p className="text-sm text-zinc-600 leading-relaxed max-w-2xl">{description}</p>
          </div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div>
                ...
                {size && <p className="text-[10px] text-zinc-400 mt-2 font-semibold">{size} {formatLabel}</p>}
              </div>

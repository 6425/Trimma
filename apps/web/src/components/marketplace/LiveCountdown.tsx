  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="min-w-[3rem] sm:min-w-[3.5rem] px-2 py-2 sm:py-2.5 rounded-lg bg-zinc-950 border border-zinc-800 shadow-[inset_0_2px_10px_rgba(0,0,0,0.65)]">
        <span className="block font-mono text-2xl sm:text-3xl font-black tabular-nums tracking-wider text-[#ffc800] leading-none">
          {value}
        </span>
      </div>
      <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-500">
        {label}
      </span>
    </div>
  );
...
function Colon() {
  return (
    <span
      className="font-mono text-xl sm:text-2xl font-black text-zinc-400 pb-5 select-none animate-pulse"
      aria-hidden
    >
      :
    </span>
...
export function LiveCountdown() {
  ...
  if (time.live) {
    return (
      <div className="mt-5 mb-1 inline-flex items-center gap-2 rounded-full bg-zinc-950 border border-[#ffc800]/40 px-5 py-2.5">
        <span className="relative flex h-2.5 w-2.5">
          <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#ffc800] opacity-75" />
          <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-[#ffc800]" />
        </span>
        <span className="text-sm font-black uppercase tracking-widest text-[#ffc800]">We&apos;re live</span>
      </div>
    );
  }

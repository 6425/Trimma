      ...
      {items.map((item) => (
        <div
          ...
          className={`flex items-center gap-2 px-2 py-1.5 rounded-lg text-[10px] font-semibold mb-0.5 ${
            item.active ? "bg-[#ffc800] text-black" : "text-white/80"
          }`}
        >
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        ...
        <div className="bg-[#ffc800] text-black text-[9px] font-bold px-3 py-1.5 rounded-lg">
          View Lead Sheet
        </div>
      </div>
    ...
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden text-[10px]">
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="flex gap-2 pt-1">
          ...
          <span className="flex-1 text-center py-1.5 rounded-lg bg-[#ffc800] text-black font-bold">
            Send to Owner
          </span>
        </div>
      </div>
      ...
      {PIPELINE_STEPS.map((step, i) => (
        <div key={step.code} className="flex items-start gap-3">
          <div className="flex flex-col items-center">
            <div className="w-6 h-6 rounded-full bg-[#ffc800]/20 border border-[#ffc800]/40 text-[#ffc800] text-[9px] font-black flex items-center justify-center">
              {i + 1}
            </div>
      ...
      {[
        ...
        <div key={a.name} className="rounded-lg border border-slate-200 bg-white p-2 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="px-2 py-1 rounded bg-[#ffc800] text-black font-bold">Save</span>
          </div>
        </div>
      ))}
    </div>
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href={path("/leads")}>
              <Button
                className={`h-9 rounded-xl bg-[#ffc800] hover:bg-[#ffc800]/90 text-black text-xs font-bold ${isRegionalHead ? "" : ""}`}
              >
                Open Salon Creation
              </Button>
            </Link>

...
/** Platform showcase salon — must stay on the public marketplace and accept bookings. */
export function isTrimmaDemoSalon(salon: {
  name?: string | null;
  slug?: string | null;
}): boolean {
  const name = (salon.name || "").trim();
  const slug = (salon.slug || "").trim().toLowerCase();
  return slug === "trimma-demo-salon" || /^trimma demo salon/i.test(name);
}
...
export function isDummySalonRecord(salon: {
  ...
  if (isTrimmaDemoSalon(salon)) return false;
